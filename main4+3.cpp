...
#define PATH_BUF_LEN 16000
#define PATH_BUF_WID 67
...
uint32_t edges[MAX_EDGE_CNT+1][3]; // 0:src, 1:dst, 2:weight
uint32_t edges_tt[4][MAX_EDGE_CNT / 2][3];
uint32_t edges_t[2][MAX_EDGE_CNT][3];
...
uint8_t path_size[THREADS][PATH_BUF_LEN];
...
void merge(uint32_t *dst, uint32_t *src0, uint32_t *src1, uint32_t *end0, uint32_t *end1);
...
inline void merge(uint32_t *dst, uint32_t *src0, uint32_t *src1, uint32_t *end0, uint32_t *end1) {
...
void *filereader(void *args) {
    ...
    munmap(buf, mmap_len);  // release buf

    // process the last line in buf (for thread 0&2)
    ...
    qsort(edges_tt[thread_id], edge_cnt, sizeof(uint32_t) * 3, cmpfunc);
...
void *filereader_merge(void *args) {
    ...
    // process the last line in buf (for thread 1&3)
    if (thread_id == 1) {  // the last line in thread 3 is always fine
        buf = (uint8_t *) mmap(NULL, BLOCK_SIZE, PROT_READ, MAP_SHARED, fd, mmap_start + mmap_len);
        for (pos = 0; pos < BLOCK_SIZE; pos = pos + 1) {
            if (buf[pos] == ',')
                ++line_offset;
    ...
    uint32_t *end0 = (uint32_t *) (src0 + t[-1].edge_cnt * 3);
    uint32_t *end1 = (uint32_t *) (src1 + t[0].edge_cnt * 3);
    merge(dst, src0, src1, end0, end1);
}
...
void fileparser(const char *fname) {
    /********** read file **********/
    fd = open(fname, O_RDONLY, 0666);  // open source file in read only mode
    struct stat sb;
    fstat(fd, &sb);
    ...
    /****** construct dsts(dst+weight), caculate child_num & father_num ******/
    uint32_t divid[5];
    divid[0] = 0;
    divid[1] = arc_cnt/4;
    divid[2] = arc_cnt/4*2;
            ...
            for (uint32_t i = fathers[next1].head; i < fathers[next1+1].head; i++) {
                ...
                if (next2 > root) {
                    if (IS_INVALID_PATH(weight2, weight1))
                        ...
                        buf[offset].weight[1] = weight2;
                        ++offset;
                }
            }
        }
    }
    qsort(buf, offset, sizeof(rPath3), cmpfunc);
    uint16_t alloc = 0;
    for (int ct = 0; ct < offset; ct++) {
        for (uint32_t i = fathers[buf[ct].path[1]].head; i < fathers[buf[ct].path[1]+1].head; i++) {
            if (srcs[i].id > root && srcs[i].id != buf[ct].path[0]) {
...
void dfs_fast(uint32_t root,uint32_t tid) {
    ...
    level_1 :
    {
        ...
        if (dfs_flag.child_offset < dfs_flag.child_num) {
            ...
            uint32_t weight = dsts[dfs_flag.child_offset].weight;
